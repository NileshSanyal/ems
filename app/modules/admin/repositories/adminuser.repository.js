...
const { Adminuser, validateAdminUser } = require('../models/admin.model');
...
var adminUserRepository = {
    ...
    saveadmin: (adminUserObj, cb) => {
        const { error } = validateAdminUser(adminUserObj);
        let errorDetails;
        ...
        if (error) {
            ...
            Adminuser.findOne({ email: adminUserObj.email }).then(adminUser => {
                if (adminUser) { 
                  return cb('Email already in use, please enter another email address' , null);

                } else {

                    let newAdminUser = new Adminuser();
                    newAdminUser.username = adminUserObj.username;
                    newAdminUser.email = adminUserObj.email;
                    newAdminUser.password = newAdminUser.generateHash(adminUserObj.password); 
                    let token = ''; 
...
<body style="-ms-text-size-adjust: 100%; -webkit-text-size-adjust: 100%;
 ...
 padding: 0'><span style="text-transform:uppercase">your account now</span></h2>
 ...
 padding-bottom: 9px; padding-left: 18px;' valign="top">
                                  ...
                                  <a href="http://localhost:3000/admin/verify/${token}?email=${adminUserObj.email}">
 ...
 padding-right:48px; padding-bottom:24px; padding-left:48px;" valign="middle">
                                        <a class="mcnButton " href="http://localhost:3000/admin/verify/${token}?email=${adminUserObj.email}" style="mso-line-height-rule: exactly;
 ...
 invitation">Activate your account</a>
                                      </td>
                                    </tr>
                                  </tbody>
                                </table>
 ...
 text-decoration: none" width="32" />
          ...
          <!-- // END TEMPLATE -->
                    ...
                    `;  
                         ...
                         /* save admin user, then send the email */
                        newAdminUser.save()
                        .then(adminUser => {

                            var email = {
                                to: adminUserObj.email,
                                from: 'no-reply@ems.com',
                            ...
                            return cb(null, 'A confirmation link has been send to your email address, please click that link to confirm your identity');
                        });
    ...
    verifyadmin: (adminUserObj, cb) =>{
      // console.log(adminUserObj.params.token);
      // console.log(adminUserObj.query.email);

      let userEmail = adminUserObj.query.email;
      let reqToken = adminUserObj.params.token;

      Adminuser.findOne({ email: userEmail }).then(adminUser => {
        if (adminUser.isVerifed === true) { 
          // console.log('False');
          // console.log(adminUser.isVerifed); 

           return cb(null, 'Your account has already been activated!!');
